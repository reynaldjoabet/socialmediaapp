  ...
  private def httpRoutes(transactor: Transactor[F]): HttpRoutes[F] =
    HealthRoutes[F].router <+> UserRoutes.make[F].router <+> LogoutRoutes[F].logoutRoutes <+>
      CommentRoutes.make[F](transactor).commentRoutes <+> PostRoutes
        .make[F](transactor)
        .postRoutes <+> LikesRoutes.make[F](transactor).likesRoutes <+> StoryRoutes
        .make[F](transactor)
        .storyRoutes <+> RelationshipRoutes.make[F](transactor).relationshipRoutes <+> Auth0Routes
        .make[F](transactor)
        .auth0Routes
...
object HttpApi {
  ...
  def csrfService[F[_]: Async]()
    ...
    crsfBulider // .withCookieName(cookieName)
      .withCookieDomain(Some("localhost")).withCookiePath(Some("/")).build.validate()
  }
  ...
  def csrfService1[F[_]: Async]() = CSRF
    ...
    .map(builder =>
      builder.withCookieName(cookieName).withCookieDomain(Some("localhost")).build.validate()
    )
