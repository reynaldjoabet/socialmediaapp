...
//Auth0JwtVerifier with a verifyAccessToken
...
// A regex for parsing the Authorization header value
   ...
   */
  private def defaultAuthFailure[F[_]](implicit
    F: Applicative[F]
  ...
  private def extractBearerToken[F[_]](
    ...
    .headers
    .get[Authorization]
    .collect { case Authorization(Token(AuthScheme.OAuth, oauthToken)) =>
      oauthToken
    }
        ...
        getBearerToken(request) match {
          case Some(value) =>
            AuthorizationService.validateJwt(value) match {
              ...
              case Success(claim)     => service(request)
